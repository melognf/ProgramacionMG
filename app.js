...
function renderTurnoView(){
  ...
  function turnWindow(dd){
    const t1 = toNum(dd.T1);
    const t2 = toNum(dd.T2);
    const t3 = toNum(dd.T3);

    const active = [];
    if (t1 > 0) active.push(1);
    if (t2 > 0) active.push(2);
    if (t3 > 0) active.push(3);
    ...
    return {
      ...
      span: active.length,
      pattern: `${t1 > 0 ? 1 : 0}${t2 > 0 ? 1 : 0}${t3 > 0 ? 1 : 0}`
    };
  }
  ...
  const sortedRows = [...rows].sort((a,b)=>{
    ...
    const wa = turnWindow(getDay(a));
    const wb = turnWindow(getDay(b));

    // 1) primero el que arranca antes
    if (wa.start !== wb.start) return wa.start - wb.start;

    // 2) si arrancan en el mismo turno, primero el que termina antes
    //    ejemplo: T1 antes que T1+T2
    if (wa.end !== wb.end) return wa.end - wb.end;

    // 3) si tienen misma ventana, primero el que ocupa menos turnos
    if (wa.span !== wb.span) return wa.span - wb.span;

    // 4) desempate final: respetar orden original del Excel
    return (a._excelOrder ?? 0) - (b._excelOrder ?? 0);
  });
